...
export function ExistingTag(props) {
    return(
        <TagContainer row>
            <Tg centered color={getContrastColor(props.data.fileColor)} background={props.data.fileColor}>{props.data.tagName}</Tg>
                <Spacer h={8}/>
                <TgTrash centered background={props.data.fileColor} pointer onClick={() => props.onDelete(props.data.id)}>
                    <DeleteIcon size={25} color={getContrastColor(props.data.fileColor)} />
                </TgTrash>
            <Spacer h={8}/>
        </TagContainer>
    );
